  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        ...
        // Optional: Remove duplicates (by student ID + courseTitle)
        const uniqueMap = new Map<string, Student>();
        res.data.forEach((student: Student) => {
          const key = `${student.id}-${student.courseTitle}`;
  ...
  return (
    <div className="container my-5">
      ...
      <div className="mb-4 d-flex flex-wrap gap-2">
        <button
          className={`btn btn-${selectedCourse === "All" ? "primary" : "outline-primary"}`}
        ...
        {courseTitles.map((title) => (
          <button
            key={title}
            className={`btn btn-${selectedCourse === title ? "primary" : "outline-primary"}`}
      ...
      {loading ? (
        ...
        <div className="alert alert-info text-center">No students enrolled yet.</div>
      ) : (
        Object.entries(filteredStudentsByCourse).map(([courseTitle, courseStudents]) => (
          <div key={courseTitle} className="card shadow-sm mb-5 border-primary">
                          ...
                          >
                            <li>
                              <button
                                ...
                                onClick={() => router.push(`/auth/Dashboard/teacherDashboard/students/${student.id}`)}
                              >
