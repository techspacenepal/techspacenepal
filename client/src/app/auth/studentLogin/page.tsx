...
const handleFirebaseGoogleLogin = async () => {
  ...
  try {
    ...
    const { data } = await axios.post("http://localhost:5000/api/students/github-login", {
      email: user.email,
      name: user.displayName,
    });
...
const handleFirebaseFacebookLogin = async () => {
  ...
  console.log("Facebook user:", user); // 👈 Add this line
    ...
    const { data } = await axios.post("http://localhost:5000/api/students/facebook-login", {
      email,
      name,
    });
    ...
    <div className="container d-flex align-items-center justify-content-center min-vh-100 px-3">
      ...
      <div className="card shadow-lg p-4 w-100" style={{ maxWidth: "400px" }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <p className="text-center">
            <Link href="/auth/studentRegister" className="text-primary text-decoration-none">← Register</Link>
          </p>
        </form>
      </div>
    </div>
