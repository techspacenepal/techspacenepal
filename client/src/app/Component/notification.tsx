...
export function Notifications() {
  ...
  // ✅ Utility: Check if date is within last 2 days
  const isWithinLast2Days = (dateString: string) => {
  ...
  // ✅ Step 2: Fetch notifications and announcements
  useEffect(() => {
    const fetchData = async () => {
      try {
        const token = Cookies.get("studentToken");
        ...
        const filteredNotifies = allNotifications.filter((n: any) =>
          isWithinLast2Days(n.createdAt)
        );

        const newNotifies = filteredNotifies.filter(
          (n: any) => new Date(n.createdAt) > notifyLastSeenDate
        );

        setNotifications(filteredNotifies);
        ...
        const filteredAnnounces = allAnnouncements.filter((a: any) =>
          isWithinLast2Days(a.date || a.createdAt)
        );

        const newAnnounces = filteredAnnounces.filter(
          (a: any) => new Date(a.date || a.createdAt) > annLastSeenDate
        );

        setAnnouncements(filteredAnnounces);
  ...
  const notifyLastSeen = new Date(
    localStorage.getItem(`lastSeenStudentNotify_${studentId}`) || new Date(0)
  );
  const annLastSeen = new Date(
    localStorage.getItem(`lastSeenAnnouncementTime_${studentId}`) || new Date(0)
  );
    ...
    <div className="position-relative me-3" ref={dropdownRef}>
      ...
      {dropdownOpen && (
        <div
          ...
          style={{ width: "350px", maxHeight: "450px", overflowY: "auto" }}
        >
          <h6 className="mb-2 text-success">🔔 My Notifications (Last 2 days)</h6>
          {notifications.length > 0 ? (
            notifications.map((notify) => {
              const isNew = new Date(notify.createdAt) > notifyLastSeen;
              return (
                <div key={notify._id} className="mb-2 border-bottom pb-2 position-relative">
                  <strong>{notify.title}</strong>
                  {isNew && (
                    <span
                      style={{
          ...
          <h6 className="mb-2 text-primary">📢 Announcements (Last 2 days)</h6>
          {announcements.length > 0 ? (
            announcements.map((a) => {
              const isNew = new Date(a.date || a.createdAt) > annLastSeen;
              return (
                <div key={a._id} className="mb-2 border-bottom pb-2 position-relative">
                  <strong>{a.title}</strong>
                  {isNew && (
                    <span
                      style={{
