...
export default function UpcommingPage() {
    ...
    const handleDateChange = (date: string, cls: ClassItem) => {
        ...
        const options = getScheduleOptions(cls).filter(opt => opt.date === date);
        const allTimes = options.map((opt) => opt.time);
        setAvailableTimes(allTimes);
        setSelectedTime(""); // reset selectedTime on date change
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await axios.post(
                "http://localhost:5000/api/upcommingregistrations/register",
                payload
            );
                            ...
                            <form onSubmit={handleSubmit}>
                                ...
                                <div className="mb-3">
                                    ...
                                    <select
                                        ...
                                        onFocus={() => { if (!selectedDate) setShowDateWarning(true); }}
                    ...
                    {classes.slice(0, 4).map((cls) => (
                        <div className="col-12 col-sm-6 col-md-3 mb-4" key={cls._id}>
                            <div
                                ...
                                onMouseEnter={(e) => { e.currentTarget.style.boxShadow = "0 0 35px rgba(0, 0, 0, 0.25)"; }}
                                onMouseLeave={(e) => { e.currentTarget.style.boxShadow = "0 0 8px rgba(0, 0, 0, 0.1)"; }}
                            >
